...
from imutils.vedio import VedioStream
import numpy as np
import argparse
import cv2
import imutils
...
if not args.get("vedio", False):
    vs = VedioStream(src=0).start()
    ...
    if len(cnts) > 0:
        ...
        for i in arange(1, len(pts)):
            ...
            if counter > 10 and i == 1 and pts[-10] is not None:
                ...
                if np.abs(dY) > 20:
                    dirY = "North" if np.sign(dY) == 1 else "Sourth"
